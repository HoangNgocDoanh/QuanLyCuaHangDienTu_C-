...
namespace OOP_QuanLyCuaHangDienTu.Sourcecode
{
    class SanPham
    {
        ...
        public void Hien()
        {
            Console.WriteLine("\t|| {0,-4} ||   {1,-50}  ||  {2,-13}  ||  {3,-8}  ||   {4,-5}  ||   {5,-10} || ", MaSP, TenSP, ThuongHieu, Giaban, SoLuong, TrangThai);
        }
    ...
    class DSSanPham
    {
        ...
        public void Them1SP()
        {
            ...
            {
                if (ds[i].MaSP.Equals(sp.MaSP))//So sánh 2 chuỗi có giống nhau hay không
                {
                    ok = true;
        ...
        public void SapHet()
        {
            ...
            for (int i = 0; i < ds.Count; i++)
            {
                if ((ds[i].SoLuong) > 0 && (ds[i].SoLuong) <= 10)
                {
                    ds[i].Hien();
                }
            }
        ...
        public void TKTen(String b)
        {
            ...
            for (int i = 0; i < ds.Count; i++)
            {
                if (ds[i].TenSP.StartsWith(b)) //Str1.StartsWith(Str2):Kiểm tra xem chuỗi Str1 có bắt đầu bằng chuỗi Str2 không?
                {
                    j = 1;
                    ds[i].Hien();
                }
        ...
        public void TKGia()
        {
            ...
            for (int i = 0; i < ds.Count; i++)
            {
                if (ds[i].Giaban >= min && ds[i].Giaban <= max)
                {
                    ds[i].Hien();
                }
            }
