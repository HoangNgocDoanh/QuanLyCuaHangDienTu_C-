...
namespace OOP_QuanLyCuaHangDienTu.Sourcecode
{
    ...
    {
        //Fields
        protected string sMaHD, sTenKH;
        protected DateTime sNgayBan;
        protected int sTongsl;
        protected float sTong;
        ...
        public string TenKH
        {
            get { return this.sTenKH; }
            set { this.sTenKH = value; }
        }
        public DateTime NgayBan
        {
            get { return this.sNgayBan; }
            set { this.sNgayBan = value; }
        }
        public int Tongsl
        {
            get { return this.sTongsl; }
            set { this.sTongsl = value; }
        }
        public float Tong
        {
            get { return this.sTong; }
            set { this.sTong = value; }
        }
        ...
        {
            return MaHD + "#" + TenKH + "#" + NgayBan.ToString("dd/MM/yyyy") + "#" + Tongsl + "#" + Tong;
        }
    }
    class CTHoaDon
    {
        HoaDon a = new HoaDon();
        //Fields
        protected string sMaHD, sMaSP;
        protected int sSoLuong;
        protected float sTongGia;
        ...
        public void HienCT()
        {
            Console.WriteLine("\t\t||{0,-4}||   {1,-4}   || {2,-10}  || {3,-10}  ||", MaHD, MaSP, SoLuong, TongGia);
        }
        public string toStringCT()
        {
            return MaHD + "#" + MaSP + "#" + SoLuong + "#" + TongGia;
        }
    }
    class DSHoaDon
    {
        ...
        public int setMa()
        {
            ...
            {
                if (max < int.Parse(a.MaHD))
                    max = int.Parse(a.MaHD);
            }
            return max + 1;
        }
        public void SuaSL()
        {
            ...
            dssp.DocTep();
            dssp.SuaHD(ct.MaSP, ct.SoLuong);
            dssp.GhiTep();
        }
        public void Them()
        {
            Console.WriteLine("\n\t\tNHẬP THÔNG TIN HÓA ĐƠN MUA HÀNG");
            hd.MaHD = setMa().ToString();
            Console.WriteLine("\t\tMã hóa đơn: " + hd.MaHD);
            hd.Nhap();
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                SuaSL();
                DSSanPham dssp = new DSSanPham();
                ct.TongGia = ct.SoLuong * float.Parse(dssp.LayGiaSanPham(ct.MaSP));
                sw.WriteLine(ct.MaHD + "#" + ct.MaSP + "#" + ct.SoLuong + "#" + ct.TongGia);
                h[i] = ct.SoLuong;
                g[i] = ct.TongGia;
            ...
            Console.WriteLine("\t\tTổng số   : " + hd.Tongsl);
            Console.WriteLine("\t\tTổng tiền : " + hd.Tong);
            ds.Add(hd);
            GhiTepHD();
        }
        ...
        public void Hien()
        {
            for (int i = 0; i < ds.Count; i++)
            {
                ...
                Console.WriteLine("\t\t Mã Hóa Đơn Nhập : {0,-5}                                              Ngày Bán: {1,10}", ds[i].MaHD, ds[i].NgayBan.ToString("dd/MM/yyyy"));
                Console.WriteLine("\t\t Tên Khách Hàng  : {0,-20}", ds[i].TenKH);
                ...
                Console.WriteLine("\t\t Tổng Số  :                                                                  {0,-7}", ds[i].Tongsl);
                Console.WriteLine("\t\t Tổng Tiền:                                                                         {0,-11}", ds[i].Tong);
        ...
        public void Xoa()
        {
            ...
            {
                if (ma != dss[j].MaHD)
                    sw.WriteLine(dss[j].MaHD + "#" + dss[j].MaSP + "#" + dss[j].SoLuong + "#" + dss[j].TongGia);
            }
            sw.Close();
        ...
        public void TKTen()
        {
            ...
            for (int i = 0; i < ds.Count; i++)
            {
                if (ds[i].TenKH.StartsWith(b))
                {
                    j = 1;
                    Console.WriteLine("\n\t\t______________________________HÓA ĐƠN BÁN HÀNG THỨ {0} TRONG DANH SÁCH________________________________\n", i + 1);
                    HienChiTiet(ds[i].MaHD);
                }
            }
            if (j == 0)
